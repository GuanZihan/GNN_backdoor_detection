...
def plot_graph_embedding(embeddings, bacdkoor_gids, labels):
    ...
    ICA = FastICA(n_components=2,random_state=1, whiten='unit-variance').fit_transform(embeddings)
    color = []
    ...
    for i in range(embeddings.shape[0]):
        if i in bacdkoor_gids:
            ...
            color.append("blue")
    plt.figure()
    plt.scatter(ICA[:, 0], ICA[:, 1], c=color)
    ...
    tps = []
    fps = []
    kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(ICA)

    if len(kmeans.labels_[kmeans.labels_ == 0]) < len(kmeans.labels_[kmeans.labels_ == 1]):
    ...
    plt.savefig("visualize_tsne_graph")
    plt.show()
    if len(kmeans.labels_[kmeans.labels_ == 1]) > 0.1 * len(kmeans.labels_):
        print(len(kmeans.labels_[kmeans.labels_ == 1]), 0.1 * len(kmeans.labels_))
        kmeans.labels_[kmeans.labels_ == 1] = 0

    y_true = np.zeros((len(list(kmeans.labels_)), 1))
    # print(type(injected_graph_idx))
    y_true[injected_graph_idx] = 1

    tn, fp, fn, tp = confusion_matrix(y_true, list(kmeans.labels_)).ravel()
    tps.append(tp / len(injected_graph_idx))
    fps.append(fp / (len(list(kmeans.labels_)) - len(injected_graph_idx)))

    accuracy = accuracy_score(y_true, list(kmeans.labels_))
...
if __name__ == '__main__':
    ...
    handler = logging.FileHandler(
        "./logs/Dataset{}+TriggerSize{}+TriggerDensity{}+Attack{}+Activation_cluster.txt".format(args.dataset, args.trigger_size,
    ...
    # backdoor_test_loader = DataLoader(backdoor_test_dataset, batch_size=1)
    train_loader = DataLoader(train_dataset, batch_size=1)
    # if args.model in ["GraphCNN"]:
    #     embeddings, labels = calculate_embedding(args, model, test_graphs)
    # else:
    #     embeddings, labels = calculate_embedding(args, model, backdoor_test_loader)


    # visualize
    # plot_node_embedding(args, embeddings, labels, 1)
    # plot_node_embedding(args, embeddings, labels, 2)
    # plot_node_embedding(args, embeddings, labels, 3)
    embeddings_graph, labels = calculate_embedding_graph(args, model, train_loader)
    print(np.array(embeddings_graph))
    plot_graph_embedding(embeddings_graph, injected_graph_idx, labels)
