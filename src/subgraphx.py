...
class PlotUtils(object):
    ...
    def plot(self, graph, nodelist, figname, title_sentence=None, **kwargs):
        ...
        elif self.dataset_name.lower() in ['muta'] + list(MoleculeNet.names.keys()):
            x = kwargs.get('x')
        ...
        elif self.dataset_name in ['ENZYMES', 'IMDB-BINARY', "Cuneiform", "AIDS", "MUTAG"]:
            x = kwargs.get('x')
            self.plot_general(graph, nodelist, x, title_sentence=title_sentence, figname=figname)
        else:
            raise NotImplementedError
    ...
    def plot_general(self,
        ...
        node_dict = {0: 'C', 1: 'N', 2: 'O', 3: 'F', 4: 'I', 5: 'Cl', 6: 'Br'}
        node_idxs = {k: int(v) for k, v in enumerate(np.where(x.cpu().numpy() == 1)[1])}
        node_labels = {k: k for k, v in node_idxs.items()}
        node_color = ['#E49D1C', '#4970C6', '#FF5357', '#29A329', 'brown', 'darkslategray', '#F0EA00']
        colors = [node_color[v % len(node_color)] for k, v in node_idxs.items()]
...
class MCTS(object):
    ...
    def mcts(self, verbose=True):
        ...
        for rollout_idx in tqdm(range(self.n_rollout)):
...
class SubgraphX(object):
    ...
    def explain_label(self, label, saved_results, x: Tensor, edge_index: Tensor, node_idx, max_nodes):
        ...
        # print("iteration {}".format(label_idx), time.time() - iteration_start)
    ...
    def __call__(self, x: Tensor, edge_index: Tensor, **kwargs) \
        ...
        # for label_idx, label in enumerate(ex_labels):
        #     p = mp.Process(target=self.explain_label, args=(label, saved_results, x, edge_index, node_idx, max_nodes))
        #     processes.append(p)
        #     p.start()
